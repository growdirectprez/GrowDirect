...
A  variety  of  benchmarks  have  been  performed  to  test  the  run-time  perfomance  of  Retek’s  forecasting
engine.   The first numbers quoted are from a Dec. 1999 joint Retek-IBM benchmark of Retek’s standard
non-promotional forecasting engine (AutoES) showing 60 million SKU/Store/Week forecasts in under an
hour.   The  second  is  a  smaller  June  2001  Promote  benchmark  showing  run-time  performance  of  the
Promote  engine  using  hardlines  sales  and  promotional  event  data.    The  last  two  benchmarks  quoted
were performed in November of 2001, specifically for a replenishment planning implementation to show
that the  forecasting  engines  could  produce  daily forecasts  using  grocery data (higher density) with little
degradation  in  run-time  performance.   Of  the  119  million  time  series  considered  over  23  million  were
active (produced non-zero forecasts).

How quickly can data be loaded into RPAS?
...
Equally  important  as  the  batch  processing  time  required  by  an  application  is  the  performance  of  data
import  routines.   Data  import  encompasses  a  variety  of  files  including  product  hierarchy  information,
location  hierarchy  information,  sales  updates,  plus  a  variety  of  other  measures  (cost  updates,  price
updates,  inventory  updates,  etc.)  depending  on  the  application.   The  RDF-IBM  import  included  only
hierarchy  and  sales  information  for  60  million  SKU/Store  intersections  and  took  40  minutes.   The  data
import for TopPlan included Stock Ledger and On Order information to support planning metrics.   In all
over 3.7GB of data (covering 1 million SKUs and 1000 stores) was extracted and loaded in 162 minutes.
The most recent benchmark was one replicating a replenishment planning sales load of 9 million distinct
SKU/Store combinations (expected weekly data load) which took right at 70 minutes.

How fast is RPAS in dealing with interactive calculations?
Planning Application Key Performance Statistics.

Performance Statistic

Mean calculation time for an edit to sales at the month level spreading to week
including updates of dependent measures and displayed aggregates.
Mean time to expanding product and calendar dimensions to display aggregate
values.
Mean time switching product using the VCR control.

Mean time to switch between tabs.
...
The above benchmark is one that was performed at the Retek Performance Center.  The purpose of this
benchmark  is  to  validate  that  RPAS’s  interactive  performance  was  sufficient  for  user  intensive
applications  such  as  planning,  where  user  interaction  with  the  data  is  fundamental  to  the  business
process.

The system used an IBM-p685 box, with 24 600 MHz processors.  The system was set up to replicate the
use by 270 users, on a system planning 600,000 style-colors.  The system was configured to contain 31
data metrics used in the planning process.
...
  RDF Key Performance Statistics
...
Create a Forecast Approval workbook

Approve a single sku/store forecast

Approve a multiple sku/store forecasts at an aggregate
product/location level.

Open an alert workbook from the Alert Manager (see
note)

Review an alert and resolve the alert.
...
“Open an alert workbook” includes opening the workbook, correcting a forecast, closing

Note:
the workbook and marking the alert as resolved.  Normally a single workbook will contain hundreds or
thousands of alerts rather than just one.

The above numbers were derived in the latest RDF benchmark (IBM P680, 12 processors) to show
interactive performance of various forecasting tasks that support the replenishment planning process.
