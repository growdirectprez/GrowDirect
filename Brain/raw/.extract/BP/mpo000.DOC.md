...
Support hard copy purchase order printing both immediately after release and through a daily batch printing job.  [No Gap]

Support the transmission of ticketing information to third parties including importers and other ticketing agents. [No Gap]

Provide the capability to record vendor receipt/acknowledgment of a purchase order either through systematic means (EDI) or manually. [No Gap]

Support the ability to re-send purchase orders with either line item changes only or as a completely new purchase order.

Provide the ability to view only that purchase order information which is critical for receiving purposes.  Selectively filter out all other information which is not important for partners/vendors to see.  
[No Gap - accomplished via Buyer/Merch Group functionality.]

 Jobs Analysis
See process map for job delegation. 

Track Open Purchase Orders (Level 3)
Requirements
Provide the ability to view corporate level on-order by units and dollars (cost and retail) for any given merchandise hierarchy, organizational hierarchy, or date range including, but not inclusive of, merchandise category, department, class, item group, region or site group.  [No Gap]

Support the ability to generate system notes which track a purchase order’s progress.  Notes will possibly be added to the purchase order at the following intervals:
Traffic notes routing instructions
DC receiving notes quantities entered, PO closed out, and in-transit
A/P notes when invoice has been received and paid
Log notes are kept when a change has been made to the order itself
[Note: Accomplished by Goods Receipts and PO Header & Line Item Text Notes.]

Provide flexible purchase order viewing options in order to track, analyze, and report on all orders.  Views must include the following:
A specific PO
All POs by status code
All POs for a major department
All POs for SKU, department
All POs for a specific vendor
All POs for a range of due dates
All POs due in a certain time period
All POs with a certain item characteristic value
All POs by location
All open orders for a major department
All open orders for a vendor
All past due orders (by vendor & store)
All orders partially received
All orders canceled within a specified time period (SAP Deleted Type)
Summary of all POs across Merchandise Hierarchy
Summary of all POs across Organizational Hierarchy
[Gap Note: Much related/workaround functionality for this req is contained in the Retail Info System.]

Provide the ability to track by SKU or shipper by providing visibility into shipper quantity and component SKUs.  Receipt of shipper should trigger inventory receipt of component SKUs not of shipper SKU.  [No Gap]

Support the daily generation of the Purchase Order Exception Report which lists:
Late shipments (including Past Due / Past Receipt dates)
Weekly receipt projections based on PO receipt dates
Orders approaching due date within tolerance
[No Gap]

Support the generation of the following Purchase Order Reports including:
POs created automatically by replenishment system
POs pending approval
POs rejected/approved
POs receiving substitutions based on vendor feedback
POs with shipments over/under order quantity based on vendor feedback

Support purchase order summary reporting that includes drill down capabilities that provide insight into detailed line Item information. 
[No Gap - handled via Customized Reports.]

Support vendor performance tracking of :
Late shipments
Over/under shipments
Substitutions
Damaged and defective merchandise
Vendor DPP performance
[Being handled as Cross functional issue with Vendor Management.]



Import Purchase Order Management

Via an external interface that periodically updates import status through a customs agency, track the clearance status of an import order.
[External interface can be supported, but data exchange requirements need to be worked and appropriate status codes need to be added to SAP. ]

Support the on-line inquiry of import item status at various points in merchandise pipeline including support for EDI Standard 214 transactions to update tracking information including:
Manufacturing lot released (QC)
Arrival at consolidator
Confirmed by consolidator
Manifested to shipper
 “On-water” in-transit
On dock
In customs
Approved
Rail / road in-transit
Container received at DC
...
Reports
Reports Added
Purchase Order Exception Report -- Printed daily for each buyer and merchandise manager shows POs which require attention.

Purchase Order Summary Reports – as follows:
POs created automatically by replenishment system
POs pending approval
POs rejected/approved
POs receiving substitutions based on vendor feedback
...
Maintain Purchase Orders (Level 3)
Requirements
Provide the ability to manually cancel any purchase order at any time.  Cancellation notices will be automatically trigger a status update to the partner/vendor involved and send a notice to transportation regarding “no pick-up” status.   [No Gap]

Provide PO maintenance capabilities that include add, cancel, modify for both header and detail line item information.  Information which may be maintained includes:
Cost (retail and markup)
Quantity
Due Dates
Cube
Weight
...
Vendor Address
Status Codes
Buyer/Inventory Manager name/number
Notes: Note can not change Vendor Name & Address via PO.  Status codes note available at this time. 

Support automatic management approval for all orders which have been maintained unless changes made exceed pre-set variance limits.  Maintained orders should follow similar approval processing as defined in Create Purchase Order.
[Note:  Dynamic preset variance levels related to OTB should be considered a gap.]

Allow for systematic maintenance of status codes based on activity.
[Systematic updates would be possible if/when Status Codes are implemented.]

Support the ability to automatically manage subsequent dates on a purchase order if any critical path dates are changed.  [No Gap]

Provide the capability to update or close  the purchase order based on one or a combination of the following factors:
Number of days past anticipated receipt date (set by item/vendor combination) 
Difference in the number of units received versus ordered (managed in total units or as a percentage of the order)
After receipt of merchandise
[Note:  Closure on differences between Units Received and on order can probably be handled by existing delivery tolerances within SAP.  No Gap]

Job Analysis
See process map for job delegation.

Manage Shipping Notices (Level 3)
Requirements
Support the electronic interface and receipt of ASNs via EDI.  [No Gap]

Support the automatic update of the purchase orders through electronic receipt of  ASNs.  [No Gap]
...
Reports Added
ASN Report - Details information received from a vendor including purchase order number, vendor, vendor due date, units shipped, and inventory manager name/number.
