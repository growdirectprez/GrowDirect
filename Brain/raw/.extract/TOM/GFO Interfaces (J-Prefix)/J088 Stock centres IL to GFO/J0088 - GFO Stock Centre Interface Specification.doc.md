...
The interface is a batch extract of Stock Centre reference data from an Integration Data Store and upload into the GFO system via integration layer. 
...
JLSCB-STOCK-CENTRE-NO PIC X(5). 
used to update SKC table

FILLER                                      PIC XX.   

Warehouse -> WH_name
JLSCB-STKC-NAME               PIC X(24).
Stock centre name.  Used to update SKC table

JLSCB-STKC-ABBR-NAME  PIC XXX.  
Abbreviated 3 character name (for example, appears on delivery list screens).  Updated on SKC table.  Is used on Display screens, reports and as part of delivery confirmation/order routing.  Direct suppliers ABBR name are maintained in CR .  Stock centres could be manually loaded in short term so SET TO SPACES.  This may need to be changed at a later date.
...
Field NameReferenced in CR?Insync formatStartLengthCOBOL FormatHeader record
Must exist
G
1
34
...
To be addressed by
1
JLSCB-STKC-ABBR-NAME – Field needs to be identified in Data Model
Business Analyst
...
Version  REF DOC_VER 0.1, Issue  REF DOC_STATUS Draft	Page:  PAGE  \* MERGEFORMAT 8 of  NUMPAGES 14	Date:  SAVEDATE \@ "d MMM yyyy" 7 Dec 2006
