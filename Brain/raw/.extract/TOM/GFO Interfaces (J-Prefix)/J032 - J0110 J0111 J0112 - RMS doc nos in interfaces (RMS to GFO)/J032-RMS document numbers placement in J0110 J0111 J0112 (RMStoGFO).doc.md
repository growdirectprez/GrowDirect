...
Introduction
Purpose of Document
The purpose of this document is to describe the interfacing requirements w.r.t  transfer files which is RMS numbed orders for the details sent by GFO for Pick By Line(PBL), Pick By store (PBS) and Direct to Stores (DTS) data between GFO (Group Forecasting and Ordering) and Retek Merchandising System (RMS) or ORMS (Oracle Retail Merchandising System)*. This document covers the three interfaces virtually marked as J0110, J0111 & J0112 by integration interfaces control team. 
...
Background
As part of Tesco Operating Model program, there is requirement to bridge the functionalities of enterprise applications including RMS and GFO. To achieve this functionality, high level process flow architecture has been designed and approved by Tesco’s Enterprise Architecture team. As part of this process flow architecture there is a set of interfaces that has been identified to be developed to transfer the file which RMS generated Numbers for GFO orders data from RMS into GFO.
...
The interface is an extract of RMS generated numbered for GFO Order data from RMS and transfers to a shared location via RTI file adaptors and uploads into the GFO system via integration layer BizTalk interfaces. The process is a delta upload in nature. The GFO batch process will identify the file based on the ile name and invoke respective sub-process. The process is a delta upload in nature. 

The BizTalk interface is meant to run at a pre-configured time interval, which on completion is expected to transfer the positional flat-file which is created by RMS in required form by GFO onto the shared location. This shared location would be monitored by GFO system at the same interval. Once the file containing the extracted data appears on the shared location, GFO system will start the processing the Data.
...
Scope
The Pro-C programs will run at a predefined time to generate the RMS numbers Records in a COBOL flat file. This flat file is transferred to a predefined location through RTI file adaptors. BizTalk picks up the COBOL flat file from the shared location for further processing and transformation.
Source Message Schema
In this context, the source carries the data in the format specified below. The data files are positional filesbut variable record in length. Each row in the data file will carry first character as definition of row. Following table carries description column, which explain at row level and column level details.

File Names: 
RMS_PBLPRCCNF_<CCYYMMDDHHmmss>_RUN<RunID>.dat - PBL order conformations – Procurement
RMS_PBLFNLCNF_<CCYYMMDDHHmmss>_RUN<RunID>.dat - PBL order conformations – Final
RMS_PBSCNF_<CCYYMMDDHHmmss>_RUN<RunID>.dat - PBS order conformations
RMS_DTSCNF_<CCYYMMDDHHmmss>_RUN<RunID>.dat - DTS order conformations

The data structure of the files remains same. The differentiation of the data processing depends upon the naming convention of the file and the data storage in the columns JIROB-PBL-IND and JIROB-PBL-TYPE-IND.

Column Name
Optional
Data Type & Length
Description
OrderHeaderIdentifier
...
Varchar2(7)
This field carries the Unit size in Character or Numeric. So, Batch which loads the data into RMS has responsibility to identify the Data type
TradTPN
...
BizTalk
Package Name: Tesco_TOM_RMS_generated_numbers_For_GFO
Stored Procedure
Name
...
Scope
BizTalk orchestration monitors files generated by RMS (with its numbering in the order files sent by GFO) arrival, which is created by Pro-C programs in the shared location for every new data file and submits the same for upload. In turn this file will be updated into GFO through COBOL program.
...
Message Format
The source message is from RMS is in the form of positional flat file and is getting copied at target location.
Message Transport Details 
...
Biztalk
Instruction Name: Tesco_TOM_Final_Store_Order_for_RMS_File_Delivery
Instruction Details
Description
To be determined
Enabled
...
GFO is the brain of TESCO ordering procedure, which will advise RMS, Storeline for the final store order of quantity. 
...
Version: 0.1, Issue  REF DOC_STATUS Draft	Page:  PAGE  \* MERGEFORMAT v of  NUMPAGES 23	Date:  SAVEDATE \@ "d MMM yyyy" 7 Feb 2007

Tesco IT Group Technology and Architecture	 TITLE  \* MERGEFORMAT Architecture



Version 0.1 REF DOC_VER , Issue  REF DOC_STATUS Draft	Page:  PAGE  \* MERGEFORMAT 19 of  NUMPAGES 22	Date:  SAVEDATE \@ "d MMM yyyy" 7 Feb 2007
