...
Introduction
Purpose of Document
The purpose of this document is to describe the requirements for the interface of Current Stock On Hand Data (SOH) from the Group Forecast and Order system (GFO) to the User Data Database (UDD) within the Integration Layer (IL). This User Data is needed by the Allocations process.
...
After End of Store Day, GFO will produce one SOH data file which the Integration Layer (IL) will immediately pass to the UDD upload directory also within the IL ready for uploading into the UDD. The file will be passed as is, without changing it’s name or data structure.
...
Architecture
GFO will publish the Current SOH data file (‘GFO_CurrentSOH.csv’) within a shared windows (CIFS) directory (‘GFO_outbox’) on the GFO host.

An RTI Shipping Agent Hosted Instruction “J010.GFO to UDD.Current SOH File Delivery” will:

Identify the Current SOH file based on name: ‘GFO_CurrentSOH.csv’
...
Scope
The integration layer purely passes the Current SOH data file from the GFO shared directory to the UDD upload directory without changing it’s name or structure.
...
J010.GFO to UDD.Current SOH File Delivery




RTI
Instruction Name: J010.GFO to UDD.Current SOH File Delivery
Instruction Details
Description
File delivery of Current Stock On Hand data file from GFO to UDD
Enabled
True
Type
Hosted
