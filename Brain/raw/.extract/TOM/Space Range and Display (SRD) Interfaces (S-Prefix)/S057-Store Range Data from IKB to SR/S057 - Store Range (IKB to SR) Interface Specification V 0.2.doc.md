...
(This Interface also covers the requirements for logical Interface S086 (Replenishment Data) from IKB to SAE (Sales Aggregation Engine)
...
24-04-2007
0.2
Covers the Requirements for Sales Aggregation Engine - S086 Replenishment Data and two additional fields requested by Store Range System.
...
S057 - Replenishment Data ( IKB to SAE) Mapping Specification  
Table of Contents
...
Introduction
Purpose of Document
The purpose of this document is to describe the interfacing requirements w.r.t Store Range data between the IKB and Store Range (SR) and the Replenishment Data between IKB and Sales Aggregation Engine (SAE). The document is of a sufficiently technical in nature to allow a developer to build an actual interface. Additionally the document contains detail that might sit logically within a Technical System Design (TSD) document, but is contained here for expediency. There is therefore no associated TSD for this interface. 

This Interface is for US and Turkey implementation.

Background
As part of the process of managing Space, Range and Display activities in store it is important to identify that the store has accurately completed all space range and display activities. This will help the centre to understand how well stores are conforming to the planograms. This interface provides Store Range Details from IKB to SR system and Replenishment Data from IKB to SAE system.
In Tesco Operating Model program, there is requirement to bridge the functionalities of enterprise applications including IKB to Store Range system and IKB to SAE system. To achieve this functionality, high level process flow architecture has been designed and approved by Tesco’s Enterprise Architecture team. As part of this process flow architecture there is a set of interfaces that has been identified to be developed for product mapping.
Scope
...
This interface extracts Store Range details / Replenishment data as a single extraction daily from IKB using a SQL Server Connection and transforms the data into respective files for Store Range and Sales Aggregation Engine. The file in turn is placed into a shared Network location. The Upload is a delta upload in nature. It is application’s responsibility to provide the deltas to Integration layer.

The batch extract would initiate the SQL Server Integration service package. SSIS package fetch the data from the holding tables of IKB. Further steps within the SSIS package would transform the data in to a file format required by two target systems i.e. Store Range system and SAE system. The file from the SSIS package is written on to the Integration file share for store range system and for SAE.
...
The interface should transfer the data for Store Range from IKB to SR and SAE file Share in the form of flat files at a pre-configured time (to be determined).  After successful completion, Target Application processes for SR and SAE would then upload the data from the respective files to the core tables of SR and SAE systems.
...
The interface should be capable of extracting data from IKB through SSIS Service layer, delivering the resulting data to into two flat files for two respective target systems (SR and SAE) before the identified cut-off time. The interface should run on scheduled batch basis. 
...
Processing Required in the SSIS Stage of the Interface for Store Range System.
Scope
This section describes the discrete steps within the SSIS package to extract and format data. The target positional flat file format is included in this section. This segment will process the data picked from IKB database (IKB Holding Table: “csg_replen_extract”) and transform into needed flat file format for the SR system.
Package Overview
...
SRP Indicator
Integer(10)
Indicates the merchandiser as decided to 'break from tray'
PlanogramLiveDate
Char(10)
Format ‘YYYY-MM-DD’
Trailer Record
...
SSIS
Package Name: TOM.S057.IKBtoSRandSAE.ReplenishmentData.dtsx
...
Processing Required in the SSIS Stage of the Interface for SAE system.
Scope
This section describes the discrete steps within the SSIS package to extract and format data. The target positional flat file format is included in this section. This segment will process the data fetched from IKB database (IKB Holding Table: “csg_replen_extract”) and transform into needed flat file format for the target system (SAE).
Package Overview
SSIS extracts the Store Range data from IKB via SQL Server OLEDB connection, at a predefined time.  In turn, the data is transformed into a flat file and is placed into a file share location.
Data Validation
N/A.
...
Object live date (planogram or floorplan as appropriate)
Date
8
Live date associated with the Planogram ID
Object end date (planogram or floorplan as appropriate)
Date
20
...
Promotional indicator (generated by this process)
Char
1

Shelf-ready packaging indicator
Char
1

Multi-located indicator
Char
1
...
To be addressed by
1
Target file share folder Names to be confirmed
...
Version 0.1, Issue  REF DOC_STATUS Draft	Page:  PAGE  \* MERGEFORMAT ii of  NUMPAGES 24	Date:  SAVEDATE \@ "d MMM yyyy" 25 Apr 2007
