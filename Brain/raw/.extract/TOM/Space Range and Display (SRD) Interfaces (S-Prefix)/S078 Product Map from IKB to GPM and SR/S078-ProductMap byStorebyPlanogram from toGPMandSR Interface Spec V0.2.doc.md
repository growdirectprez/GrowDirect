...
Mapping spreadsheet
S078 - Product Map by Store by Planogram (IKB to GPM) Mappings.xls
...
The interface will run once a day, on scheduled basis, and consists of a SQL Server Integration Services package that extracts data from the IKB through stored procedures, and transforms and formats it into the target file format for store range and target table format for GPM.

For GPM, the interface should transfer the data for Map of Products by Store by Planogram from IKB to GPM holding table at a pre-configured time (to be determined). After successful completion, another GPM process would then upload the Product Map data to the core tables of GPM.

For Store range, the file is delivered to Store range system to a folder on the host. Since source and target systems are in windows platform the SSIS package should deliver the file directly on the store range system host.
...
The interface should be capable of extracting data from IKB through SSIS Service layer using SQL Stored Procedure, delivering the resulting data to Holding Table to GPM (name TBD) and flat file to store range before the identified cut-off time. The interface should run on scheduled batch basis. 
...
Scope
A scheduled batch job/s runs, at a pre-configured time, to extract the Product map by store by planogram data from the IKB holding table (csg_product_data). This batch job in turn kicks off a process that executes SQL server Integration Services package, calls the stored procedure to pick data from IKB.  The retuned data-set will be transformed to the required format within SSIS package and written to a file share of the store range system and holding table of GPM.
...
Scope
This section describes the discrete steps within the SSIS package to extract and format data. The target schema is included in this section. This segment will process the data picked from IKB database and transform into needed format which is required by GPM holding table. After processing, transformed data will get loaded into GPM holding table (name of the holding table to be determined). In turn the holding table data will get loaded into tbl_ProductPlanogram in GPM database.
Package Overview
SSIS extracts the product reference data from IKB via SQL Server OLEDB connection, at a predefined time. In turn, the data is inserted into an Intermediate table at GPM.
Data Validation
N/A.
Filtering
...
Yes
Unique Identifier for  planogram
StoreID
Integer

Yes
This will contain the store id
ModNumber
Varchar
50
No
Mod label in the store
ShelfId
Integer

No
Shelf number on the mod
ProdPositionNo
Integer

No
This field will give the position of the product on the shelf
SKUID
Varchar
50
Yes
Item number i.e. Tesco specific product number
FacingWide
Integer

Yes
This will give the no of products across the width of the shelf
FacingsHigh
Integer
...
Yes
This will give the no of products across the breadth of the shelf
IsMultilocated
Bit

No
This field will tell whether the product is multilocated or not
PlanogramLiveDate
Datetime

No
Date when Planogram goes active in a store
...
Scope
This section describes the parallel steps within the SSIS package to extract and format data to another target schema. The target schema is included in this section. This segment will process the data picked from IKB database and transform into flat file format which is required by Store range system. 
Data Validation
Not Required
...
Version 0.1 REF DOC_VER , Issue  REF DOC_STATUS Draft	Page:  PAGE  \* MERGEFORMAT 19 of  NUMPAGES 22	Date:  SAVEDATE \@ "d MMM yyyy" 16 Mar 2007
