...
Scope
A scheduled batch job/s runs, at a pre-configured time, to extract the Product map by store by planogram data from the IKB holding table (csg_product_map) by executing SQL server Integration Services package. The retuned data-set will be transformed to the required format within SSIS package and written to a file share of the store range system and holding table of GPM.
...
Version 0.1 REF DOC_VER , Issue  REF DOC_STATUS Draft	Page:  PAGE  \* MERGEFORMAT 22 of  NUMPAGES 22	Date:  SAVEDATE \@ "d MMM yyyy" 16 Mar 2007
