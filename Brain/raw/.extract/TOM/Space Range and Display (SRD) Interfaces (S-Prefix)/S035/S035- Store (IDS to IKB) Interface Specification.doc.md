...
File Name: Tesco - New Store Openings Interface v1 0.doc
...
There is no source message as such. The source data resides in the form of RDBMS tables. The data is extracted via Store Common form. 

Sample Message Format
The source data flows from IDS via SSIS using Store Common Form. Sample message is not required.
...
Assumption
1
Data type and length of fields of destination table Store Holding Table are based on ix_str_store table in IKB.
...
To be addressed by
1
BSDs is not signed off. Requirements may change.
Samantha Baptiste
2
Need to have a clear understanding of how the common form works
SysRepublic
3
Store Holding Table generation script to be provided. Using dummy Store holding table (name, datatypes and size based on ix_str_store table) may result in rework if there are significant changes when final script issued from JDA
JDA
...
Version  REF DOC_VER 0.1, Issue  REF DOC_STATUS Draft	Page:  PAGE  \* MERGEFORMAT 12 of  NUMPAGES 20	Date:  SAVEDATE \@ "d MMM yyyy" 12 Feb 2007
