...
Introduction
Purpose of Document
The purpose of this document is to describe the interfacing requirements w.r.t Department reference data between the IDS to IKB. The document is of a sufficiently technical nature to allow a developer to build an actual interface. Additionally the document contains detail that might sit logically within a Technical System Design (TSD) document, but is contained here for expediency. There is therefore no associated TSD for this interface. 

This interface is for US implementation.

Background
As part of Tesco Operating Model program, there is requirement to bridge the functionalities of enterprise applications including IDS and IKB. To achieve this functionality, high level process flow architecture has been designed and approved by Tesco’s Enterprise Architecture team. As part of this process flow architecture there is a set of interfaces that has been identified to be developed to transfer Department Reference data from IDS into IKB.
Scope
...
The interface is an extract of Department Reference Data from IDS using Common Forms and uploads into the IKB Department Holding Table using SSIS. The Upload is a full upload in nature.

The interface is meant to run nightly at a pre-configured time, which on completion is expected to upload the Department data in a Department Holding Table in IKB.
...
The interface should be capable of extracting data from IDS using Common Forms, delivering the resulting data to IKB Department Holding Table before the identified cut-off time. The interface should run on scheduled batch basis. 
...
SSIS extracts the Department reference data from IDS using Common Forms, nightly at predefined time.  In turn, the data is inserted into an Intermediate table at IKB which is the Department Holding Table. 
Source Message Schema
There is no source message as such. The source data resides in the form of RDBMS tables. The data is extracted via Merchandise Hierarchy Common form. 

Sample Message Format
The source data flows from IDS via SSIS using Merchandise Hierarchy Common Form. Sample message is not required.
...
Assumption
1
Data type and length of fields of destination table Department Holding Table are based on Department Common Form.
...
SysRepublic
2
Department Holding Table generation script to be provided . Using dummy Department holding table (name, datatypes and size based on Department_Common Form) may result in rework if there are significant changes when final script issued from JDA
JDA
...
Version  REF DOC_VER 0.1, Issue  REF DOC_STATUS Draft	Page:  PAGE  \* MERGEFORMAT 20 of  NUMPAGES 20	Date:  SAVEDATE \@ "d MMM yyyy" 12 Feb 2007
