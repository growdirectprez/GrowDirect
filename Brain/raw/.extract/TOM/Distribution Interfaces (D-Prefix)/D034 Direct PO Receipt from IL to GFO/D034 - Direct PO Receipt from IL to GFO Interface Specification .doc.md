...
Introduction
Purpose of Document
The purpose of this document is to describe the requirements for the interface of PO (Purchase Order) Receipt data, for direct deliveries only, from the Integration Layer (IL), to the Group Forecast and Ordering (GFO) system.
...
This interface is for Turkey and the US.
...
This interface details the delivery of the PO Receipt from the IL to GFO.
...
The PO Receipt message, existing as a correlated orchestration within the IL, will be converted to a format suitable for upload by GFO, and placed in a shared directory on the GFO host ready for upload by GFO.

The successful placement of the file into the shared directory on the GFO host will be logged using the Trackpoint component of the IMOF framework:
...
Audit Requirements
The interface will use the Trackpoint component of IMOF to record the successful file transfer to the GFO shared directory.
...
The interface should be capable of transporting the PO Receipt file to the GFO host.
...
- Transform the PO Receipt (existing as a correlated orchestration) to the format suitable for GFO upload,
...
- Place the file in the shared directory ‘GFO_inbox’ on the GFO host.
...
Message Format
Fixed record width, positional, flat file. 
...
Failure of file creation or delivery to be raised through Error Processing component of IMOF.
...
GFO
Group Forecasting and Ordering System.
...
Version  REF DOC_VER 0.1, Issue  REF DOC_STATUS Draft	Page:  PAGE  \* MERGEFORMAT 20 of  NUMPAGES 21	Date:  SAVEDATE \@ "d MMM yyyy" 26 Feb 2007
