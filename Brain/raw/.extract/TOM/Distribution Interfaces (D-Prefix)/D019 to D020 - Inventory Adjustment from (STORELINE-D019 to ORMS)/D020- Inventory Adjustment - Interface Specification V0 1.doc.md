...
The interface will be governed by BizTalk Orchestration, which expects the Inventory Adjustment data (| delimited flat file) from STORELINE interface. The store line interface will put this file (*.dat) in the shared location through RTI file adapter. The BizTalk server then transfers this file through RTI adopter and process to generate XML file (containing Inventory Adjustment data records). The XML file then put into the ORMS shared location through FTP from which RIB will take into ORMS.
...
Version  REF DOC_VER Error! Reference source not found., Issue  REF DOC_STATUS Draft	Page:  PAGE  \* MERGEFORMAT 8 of  NUMPAGES 23	Date:  SAVEDATE \@ "d MMM yyyy" 31 Jan 2007
