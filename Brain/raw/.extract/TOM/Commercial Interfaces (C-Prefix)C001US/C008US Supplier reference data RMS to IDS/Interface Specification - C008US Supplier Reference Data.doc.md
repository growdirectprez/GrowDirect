...
Interface Specification C008US
Supplier Reference Data from RMS to Integration Layer
...
Author:Paul GardnerDate:
 DOCPROPERTY "Doc Issue Date"  \* MERGEFORMAT 10-Jan-2007
Version:
0.2 
Status:
...
Background
The purpose of the interface is to supplier reference data from Oracle Retail Management System (RMS) via the Retail Integration Bus (RIB). These messages will be mapped to the Tesco common form and copied to the Integration Data Store (IDS).
Scope
...
This interface is the transmission of supplier reference data from RMS to the integration layer.
...
Scope
When the supplier reference data is received by the integration layer, these details will be mapped to the Tesco common form and stored in the integration data store.
...
The common form schema for Warehouse can be found in .\Common Form Schemas\Supplier.xsd.
Message Transport Details 
...
10 Jan 2007
V0.2 Draft
Updated mappings based on new data model
...
Version  REF DOC_VER 0.1, Issue  REF DOC_STATUS Draft	Page:  PAGE  \* MERGEFORMAT 12 of  NUMPAGES 12	Date:  SAVEDATE \@ "d MMM yyyy" 3 Jan 2007
