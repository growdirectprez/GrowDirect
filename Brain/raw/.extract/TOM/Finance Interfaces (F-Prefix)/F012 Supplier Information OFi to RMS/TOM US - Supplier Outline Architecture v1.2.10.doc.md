...
14/03/2007
1.2.10
Feedback from HSC incorporated
...
Write extracted file from Oracle Financials to a CIFS share 
...
Oracle Financials script extracts a  Supplier flat file to a CIFS share which is also a Biztalk File Receive location. Biztalk converts the file to a Common Form then to RIB format and loads it to ORMS via the RIB. A SQL Supplier database in the Integration Layer will also be loaded using the Common Form during this process. 
...
The extraction will be scheduled for an hourly run. (If Vendors are suspended we will want the information propogated fairly rapidly). 
The extract will comprise all records changed since the last extract.
File Extraction Schedule (Application Project)
The file will be extracted to a CIFS based share. 
An hourly schedule will be set up to run the extraction. This may change if the Business requests a change.
...
Version  DOCPROPERTY "Doc Version"  \* MERGEFORMAT 1.0 REF DOC_STATUS 	Page:  PAGE  \* MERGEFORMAT iii of  NUMPAGES 48	Date:  SAVEDATE \@ "d MMM yyyy" 13 Mar 2007
...
Version  DOCPROPERTY "Doc Version"  \* MERGEFORMAT 1.0 REF DOC_STATUS 	Page:  PAGE  \* MERGEFORMAT 10 of  NUMPAGES 48	Date:  SAVEDATE \@ "d MMM yyyy" 13 Mar 2007
...
Version  DOCPROPERTY "Doc Version"  \* MERGEFORMAT 1.0 REF DOC_STATUS 	Page:  PAGE  \* MERGEFORMAT 48 of  NUMPAGES 48	Date:  SAVEDATE \@ "d MMM yyyy" 13 Mar 2007
