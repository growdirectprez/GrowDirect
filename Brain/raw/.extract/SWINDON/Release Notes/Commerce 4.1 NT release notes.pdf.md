...
• /bexpress/scripts/login.jsp

• /mradmin/scripts/login.jsp
...
• Conﬁguring a named application
...
• Using the Business Express sample data

• Purging contents for deleted visitors

After installing and conﬁguring One-To-One Commerce, install Commerce for Command Center,
the business-to-business commerce addition to the One-To-One Command Center application. See
“Commerce for Command Center installation” on page 36.
...
3. Run setup.exe from the windows\Business directory. This will install One-To-One
...
l “Conﬁguring the HTTP server” on page 20

Conﬁguring One-To-One

Make the following conﬁguration changes to One-To-One. These changes allow you to start the
system and use the One-To-One Commerce Business Express sample application.
...
1. Deﬁne“BusinessExpress” as an additional service to use the sample application data. Use this

example to deﬁne your own services.

service BusinessExpress {
parameter

email_address = "manager@BusinessExpress.com"
postal_address = "1100 First St, San Jose, CA"
www_url = "www.BusinessExpress.com"
phone_number = "800-555-1010"
fax_number = "800-555-2020"
pmttype_id = "0, 1, 2, 10001, 10002"
...
2=cybersourceics2, 10001=po, 10002=po"
...
MR_PRICE_LIST_CACHE_SIZE=

"10"

MR_PRICE_LIST_CACHE_FLUSH=

"1"

MR_SELECT_LIST_CACHE_SIZE=

"10"

MR_SELECT_LIST_CACHE_FLUSH= "1"
...
Number of pricing program lists
cached in memory per
interaction manager.

If set to "1" the pricing list
cache is ﬂushed after the
pricing list is rebuilt.

Number of product lists cached
in memory per interaction
manager.

If set to "1" the selection list
cache is ﬂushed after the
selection list is rebuilt.
...
"http://host:port/cgi-bin/inetcgi/
mrcatalog/scripts/login.jsp"
...
Default pricing and product
selection URL. Specify this to
run the pricing and product
selection utility in the
One-To-One Command Center.
The interaction manager must
be running for this to work.

Default admin tool URL.
Specify this to run the admin
utility in the One-To-One
Command Center. The
interaction manager must be
running for this to work.

If set to zero, the catalog OID
does not appear.

Default number of rows per
page displayed.
...
MR_PAYMENT_METHOD=0, MR_ORDERS=0, PRICING_PGM=0, SELECTION_PGM=0,
...
Also, see “Conﬁguring AVP” on page 27 of these release notes.
...
visitors can review the items in theirrequisitions.
...
l The path to your script directory. The setup process creates bexpress/ as a subdirectory in the
script root directory, and then copies the One-To-One Commerce script and image ﬁles into that
subdirectory. The setup process also copies the bexpress.html ﬁle to the document root
directory, and creates and sets up the mradmin and mrcatalog applications.
...
ﬁles for your site. The utility creates a subdirectory (bexpress/scripts) in the script root
...
and loads the sample script ﬁles and images into <script root>/bexpress/scripts and
...
<doc root>/bexpress/images. If commerce_setup cannot ﬁnd your <doc root>
directory, you’ll have to copy the ﬁles manually.

5. Edit the access control list ﬁle located at $BV1TO1_VAR/BVSNsmgr/"hostname"/bvsm.ACL.
The entries in the ﬁle erroneously contain a preceding slash, which you must remove manually.
For more information, see number 08905 under “Known problems and issues” on page 6.

6. Enable the order management daemons. Edit the bv1to1.conf ﬁle, changing the value of the
...
7. Restart One-To-One with bvconf.
...
the service is deﬁned in your bv1to1.conf ﬁle along with the Business Express service.
...
System Administration Guide. If you are creating a named application see “Conﬁguring a named
...
bexpress.html, include target="_top" to remove the frame set. Otherwise, the welcome
...
If you are running a named application, using a gateway program copied from $BV1TO1/bin/
anamecgi, that you set up during imgr_conf, you must make the following editions:

1. The ACL ﬁle created during imgr_conf does not contain the access control list entries for the
mradmin and mrcatalog applications. Copy the access control list entries from $BV1TO1_VAR/
BVSNsmgr/"hostname"/bvsm.ACL and append them to your ACL ﬁle ($BV1TO1_VAR/
"yourhostname"/yourName.ACL). Copy the following line (note this is all one line, your
entries should also occupy one line):
...
/mradmin/scripts/ma_ar_ca/* [Merchant_Administrator]+:
[Account_Representative]+:[Corporate_Administrator]+:@ALL-

2. Edit the access control list entries you just copied. The entries in the list erroneously contain

preceding slashes ‘/’ for each entry, which you must remove manually. For example,
...
3. Edit the mradmin.html and mrcatalog.html ﬁles to work with the new gateway

application:

/cgi-bin/inetcgi.exe

Edit the directives in the mradmin.html and mrcatalog.html ﬁles so they access your
named gateway application:

/cgi-bin/yourName.exe

See the Installation and System Administration Guide for more information about creating a named
gateway application.
...
merchant/bexpress. To make the HTML, scripts and graphics ﬁles available to your HTTP
server, the commerce_setup script copies all the ﬁles in that directory into a subdirectory called
bexpress in your document root directory. Additionally, it further copies the bexpress.html
into your document root.

$BV1TO1/merchant/bexpress/
<script root>/bexpress/scripts/
<HTTP document root>/bexpress/images
<HTTP document root>/bexpress.html
...
To make bexpress.html the default page for your site, see your HTTP daemon documentation.
Sometimes it is as easy as creating a ﬁle named index.html in the HTTP document root directory,
like this:

cd <HTTP document root>
cp bexpress.html index.html

Restart your HTTP daemon to recognize these changes.

The installation is complete. You can now start the One-To-One Commerce BusinessExpress sample
application. See “Using the Business Express sample data” on page 34.
...
Command Center, or log in to the BusinessExpress sample application after you’ve installed and
...
The default installation and setup creates an application called BusinessExpress. The easiest way to
...
2. Near the end of the bv1to1.conf ﬁle, look for BusinessExpress service deﬁnition and
...
3. Review “Supplemental information” on page 27 for additional conﬁguration tasks that might

apply to your application.

You can now proceed to customize the BusinessExpress application.

To customize the BusinessExpress application for your site:

1. Customize the e-mail address, subject, to, and from ﬁelds for a conﬁrmation e-mail by editing

the bexpress/scripts/order/confirmation.jsp script.
...
l “Conﬁguring CyberSource” on page 28

l “Conﬁguring the Shipping methods” on page 29

l “Using the Business Express sample data” on page 34

l “Purging contents for deleted visitors” on page 35
...
described in “Exceptions” on page 32. See that description for details.
...
See “Conﬁguring the HTTP server” on page 20 to set up your HTTP server to work with the sample
application.

To use the Business Express sample application, point your browser to /bexpress/scripts/
bexpress.html or index.html under your script root directory.

Two sample corporate accounts are established in Business Express.
...
You can create additional visitors for these accounts as the account representative in the account
admin utility.
...
Only the merchant administrator, designated during commerce setup, can create additional
accounts. See the One-To-One Commerce User’s Guide and One-To-One Commerce Developer’s Guide for
more information.

Purging contents for deleted visitors

Periodically you’ll want to remove the purchasing lists and quotes that pertain to deleted visitors.
The following SQL statements accomplish these tasks.

Back-up your database before running these statements.

Purging purchasing lists
...
One-To-One Business Commerce Version 4.1.0 for Windows NT Installation and Release Notes
Commerce for Command Center installation

Commerce for Command Center installation

This section provides installation pre-requisites and describes how to install and conﬁgure
Commerce for Command Center—a Windows based addition to the BroadVision One-To-One
Command Center. As part of the One-To-One Commerce application, Commerce for Command
Center supports account, product catalog, and contract management for One-To-One business-to-
business electronic commerce Web sites. Commerce for Command Center also provides you with
access to the Account Admin and Contract and Program Utilities. Both are HTML graphical user
interfaces that are delivered and installed with the One-To-One Commerce application.

 The One-To-One Command Center must be installed on each PC that connects to a
One-To-One interactive service before you install Commerce for Command Center. Be sure to
install Commerce for Command Center on every PC that runs the One-To-One Command
Center.

The Commerce for Command Center installation and conﬁguration instructions include these
topics:

l “System and software requirements” on page 36.”

l “Before you install” on page 36.”

l “Installing Commerce for Command Center” on page 37.”

System and software requirements

Installing and using the Commerce for Command Center requires:
...
l Version 4.1 of the One-To-One Command Center installed on your computer as described in the

One-To-One Command Center Release Notes for Version 4.1.

l One-To-One Command Center Patch 4.1C installed on your computer after installing the

Command Center. See “Before you install” on page 36 for more information.

Before you install

Before you install Commerce for Command Center be sure that the following conditions have been
met.

Verify the following information

l Verify that Internet Explorer 5.0 is installed on your computer.

l Ask your system administrator for the name of the One-To-One Root Host server that the
One-To-One Command Center communicates with, and its IT_DAEMON_PORT number.
...
One-To-One Business Commerce Version 4.1.0 for Windows NT Installation and Release Notes
Logging in and setting the administrator password

37

l Verify that the One-To-One Command Center Version 4.1 is installed and initialized on your

network. If it’s not, you can ﬁnd the installation and conﬁguration instructions for this
application in the Version 4.1 One-To-One Command Center Release Notes.

l Verify that the One-To-One Command Center Patch 4.1C has been installed after Version V4.1 of
the One-To-One Command Center. If it hasn’t been, install the patch now. This patch ﬁle is
named BVSN4-1c.zip, and is located on the Version 4.1.0 One-To-One Commerce CD-ROM .
Refer to “Installing One-To-One Command Center patch C” on page 37 for instructions.

Installing One-To-One Command Center patch C

Installing this One-To-One Command Center BVSN4-1c.zip patch is required to run the
Commerce for Command Center addition. If the patch is not already installed, proceed as follows:

1. If the One-To-One servers or Interaction Manager servers are running, shut them down now:

$BV1TO1/bin/imgr_conf -a stop
$BV1TO1/bin/bvconf shutdown

2. Locate the One-To-One Command Center Version 4.1 installation directory. The default location

is C:\Program Files\BroadVision\Command Center 4.1.

3. Unzip the patch ﬁle from \windows\DCCv4.1c\BVSN4-1c.zip on the One-To-One
Commerce Version 4.1.0 CD-ROM into the One-To-One Command Center Version 4.1
installation directory on your computer. The patch is also available at the Broadvision web site:
www.broadvision.com.

Installing Commerce for Command Center

To install the Commerce for Command Center:
...
2. From the One-To-One Commerce CD in your CD-ROM drive, or from the One-To-One

Commerce software on your network, run setup.exe from the \windows\DCC_Commerce
directory.

3. Follow the instructions on your screen.

Running setup.exe automatically installs Commerce for Command Center in the appropriate
directories. You cannot determine these directories.

Logging in and setting the administrator password

To perform this task and most of the others described in this document, log into the One-To-One
Command Center and the interactive service you want to work with using the merchant
administrator account. This is a special Command Center account with administrative permissions.
...
This topic assumes that the required software has been installed on your computer as described in
“System and software requirements” on page 36 and “Installing Commerce for Command Center”
on page 37.

These topics help you begin your merchant administrator tasks:

l “Starting the Command Center” on page 38.

l “Working with the One-To-One Root Host server connections” on page 38.

l “Logging in as the merchant administrator” on page 39.

l “Changing the administrator password” on page 40.

l “Working with interactive services” on page 40.

Starting the Command Center

To start the One-To-One Command Center, choose Programs|BroadVision from the Windows Start
menu. The ﬁrst time you start the application, it displays the Options dialog where you conﬁgure
the host and port connection. The host is the One-To-One Root Host server that the Command
Center communicates with. To conﬁgure the host, you need the enter the name of the host server
and the Orbix daemon port number for that server.

Working with the One-To-One Root Host server connections

You initially conﬁgure the host and port connection in the Options dialog, which is automatically
displayed the ﬁrst time you start the Command Center. Later, you can modify this connection,
create additional connections to different servers, and delete connections in this dialog. To open the
Options dialog in the future, select Tools|Options menu command.

Working with the Root Host server includes:

l “Conﬁguring the Root Host server and port connections” on page 38.

l “Modifying a host connection” on page 39.

l “Deleting a host connection” on page 39.

l “Setting the Connection Retries” on page 39.

Conﬁguring the Root Host server and port connections

Set up and modify the root host server and port connections in the Options dialog.
...
5. Repeat Step 1 through Step 4 for each host and port connection you want to add to the list.
...
2. From the list of connections, select the one you want to modify.

3. Make the necessary changes to the Description, Host, and Port.

4. Click Modify.

5. Click OK when you ﬁnish.
...
2. From the list of connections, select the one you want to delete.

3. Click Delete.

4. Click OK when you ﬁnish.
...
When you log in for the ﬁrst time, after conﬁguring the host and port connection clicking OK in the
Options dialog opens the login window. After the ﬁrst login, starting the application displays the
login window. The information that you type in the login window is case sensitive.
...
2. In the User Name ﬁeld, enter mradmin.

3. In the Password ﬁeld, enter immradmin.

4. Click Login.

 If it takes a long time to connect to the host server when you log in, you may need to increase
the Connection Retries number on the Connection tab in the Options window. To open this
window, select Tools|Options. Refer to “Setting the Connection Retries” on page 39 for
instructions.

5. After you log in with this user name and password, change the administrator user name and
password so that others can’t log in as the administrator. Go to “Changing the administrator
password” on page 40 for instructions.

Changing the administrator password

Use the Modify User dialog for the Command Center administrator account to change your
password for the service you’re logged into. To change your password for a different service, you
need to exit the current service, and log into a that one.

 Your password is case sensitive.

To open the Modify User dialog, select the Users module in the Tools group. Choose User|Modify
to open the Name and Password tab of the dialog.

To change you password:

1. Enter your New Password.

2. Verify the new password by entering it again in the Repeat Password ﬁeld.

3. Click OK.

Working with interactive services

One-To-One Command Center administration work, such as creating accounts, applies to one
interactive service—the service you select when you log in. You can change to a different service,
update the list of services with current information, and exit a service without closing the Command
Center.

Working with interactive services includes:

l “Changing to a different service” on page 40.

l “Refreshing the list of services” on page 41.

l “Exiting a Service” on page 41.
...
One-To-One Business Commerce Version 4.1.0 for Windows NT Installation and Release Notes
Exiting the One-To-One Command Center
...
Administering Commerce for Command Center

When One-To-One Commerce Version 4.1.0 and Commerce for Command Center have been
installed and conﬁgured, go to Appendix A, “Administering Commerce for Command Center” in
the One-To-One Commerce User’s Guide. This appendix explains how to set up user accounts, assign
user permissions, and begin working with Commerce for Command Center.
...
2. Remove the directories and ﬁles under the HTTP doc root:
...
3. Remove the doc root ﬁles:

<HTTP doc root>/bexpress.html.

<HTTP doc root>/mradmin.html.

<HTTP doc root>/mrcatalog.html.

4. Remove the directories and ﬁles under the script root:
...
5. Remove the directory and ﬁles under $BV1TO1_VAR/etc/shipping.conf

6. Remove the service BusinessExpress from bv1to1.conf (and WebStore if applicable).
...
One-To-One Business Commerce Version 4.1.0 for Windows NT Installation and Release Notes
Removing Commerce for Command Center
...
You can remove Commerce for Command Center using the Add/Remove feature in the Windows
Control Panel. Removing Commerce for Command Center does not affect One-To-One Command
