...
date: 2026-05-01
type: raw
source: Brain/raw/.extract/Ross/Technical_Design_Ross_Alerts_and_Monitors_v3.doc.md
tags: [ross, rti, store-transfer, archive-2011, tech-design]
project: 
status: unprocessed
---

# Technical_Design_Ross_Alerts_and_Monitors_v3.doc

## Source
File: `Brain/raw/.extract/Ross/Technical_Design_Ross_Alerts_and_Monitors_v3.doc.md`
Size: 19,408 bytes
...
INTRODUCTION
Purpose
This document describes the Standard RTI components which should be monitored for the File Transfer HUB, and provides a description of the Standard alerts that an RTI agent will generate for the Windows Event Log. This document also covers any assumptions/pre-requisites for the infrastructure as well as identifying the specific areas where monitoring would be required.
...
Document Scope
This document will cover the Standard errors and alerts the base components of the RT solution will generate.  It is not intended to describe the specific error conditions that may occur related the functional alerting requirements of a specific interface or alerting conditions for the DDS Hub.  The Specific Details of those requirements should be addressed as part of each interface functional interface or file transfer specification.
Audience
...
SQL Agent Job
CMC Databse – Service Pulse Alert
RTI
Alert if store does not communicate to Hub with x seconds / minutes
...
All DDS Alerts generated at System or Instruction level will be written to the Windows Application Event Log using the Event Log adapter native to RTI.  In addition, some Alerts will have the capability of raising Alert messages back to SOA via the BPEL Callback Web Service.  The two Alerts types are described below:
...
Notes
Category ID
The category of Alert for RTI values would include RTI Infrastructure, RTI Application
To be used as a key routing field by the CA application to the appropriate Support team
Source
Architecture or Functional component which is threw the alert (Agent, DDS, RMS, EDW, etc.)
Used as key filed for routing by CA
Severity
Standard windows severity levels (Error, Warning, Information)
Only Errors will generated CA support processes
Event ID
A specific event id of the error message

Machine Name
The machine from which the alert originated

Description
A text description of the error / alert condition
...
In order to limit the amount of monitoring and CA alert configuration on the Store ISP, the CMC will be the primary method of determining a potential error condition on the Store Agent.  A service pulse alert instruction will be created to monitor the CMC for store service pulses. If the CMC has not logged a service pulse form the store within the agreed amount of time (5 minutes), the instruction will raise an Alert to the Event log on the Hub Database Server.

Support personnel will be responsible for troubleshooting the Service Pulse failure by looking to the Store ISP event log to determine the Agent error or network condition that may have been preventing the agent from communicating with the hub.  The system alerts generated in the Store will be the same as the hub agent alerts listed in section 5 above.
...
©2007 Accenture. All Rights Reserved.	 PAGE 1                                 DATE \@ "M/d/yyyy" 6/1/2011
