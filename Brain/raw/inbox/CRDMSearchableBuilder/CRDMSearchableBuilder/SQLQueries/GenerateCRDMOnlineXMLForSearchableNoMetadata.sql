...
'SELECT [name] ' +
  'FROM sys.tables ' +
 'WHERE [name] IN (''OrderHeader'') ' +
   'AND SCHEMA_NAME(schema_id) = ''online'' ' +  
 'ORDER BY [name] DESC'
...
'SELECT [name] ' +
  'FROM sys.tables ' +
   'WHERE [name] NOT IN (''OrderHeader'') ' +
   'AND (SCHEMA_NAME(schema_id) = ''online'' OR SCHEMA_NAME(schema_id) = ''customer'') ' +
 'ORDER BY [name]'
...
BEGIN
	...
	'SELECT c.[name], p.[name], ' + CAST(@iter AS NVARCHAR(10)) +
	 ...
	 'WHERE t.[name] = ''' + @tableName +
	   ''' AND (SCHEMA_NAME(t.schema_id) = ''online'' OR SCHEMA_NAME(t.schema_id) = ''customer'') ' +
	   'AND p.[name] != ''sysname'' ' +
	...
	BEGIN
		...
		BEGIN
			...
			BEGIN 
				...
				IF @columnName IN ('OrderID', 'CustomerOrderID', 'CustomerID', 'DateCreated', 'DateUpdated', 'OrderStatus', 'Amount')
				SET @isDefaultDisplayColumn = 1
			END
			ELSE
			BEGIN
				IF @columnName IN ('OrderID', 'CustomerOrderID', 'CustomerID', 'DateCreated', 'DateUpdated', 'OrderStatus', 'CheckPointID' )
				SET @IncludeInHadouken = 0
			END
...
SELECT A.TableID AS "@TableId", A.TableName AS "@TableName", A.TableAlias AS "@TableAlias",
 ...
 ORDER BY A.TableID
   FOR XML PATH('Table'), ROOT('Sysrepublic.CRDMOnline'), TYPE
