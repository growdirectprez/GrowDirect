...
'SELECT [name] ' +
  'FROM sys.tables ' +
 'WHERE [name] IN (''CRDM_Header'', ''CRDM_FastFact'') ' +
   'AND SCHEMA_NAME(schema_id) = ''dbo'' ' +
   CASE WHEN @populatedTableExists = 1 
		THEN 'AND [name] COLLATE Latin1_General_BIN IN (SELECT TableName COLLATE Latin1_General_BIN FROM [dbo].[Populated] WHERE [ColumnName] IS NULL AND [Populated] = 1) '
		ELSE ''
	END +
 'ORDER BY [name] DESC'
...
'SELECT [name] ' +
 ...
 'WHERE [name] LIKE ''CRDM_%'' ' +
   'AND [name] NOT IN (''CRDM_Header'', ''CRDM_FastFact'',''CRDM_Receipt'') ' +
   'AND SCHEMA_NAME(schema_id) = ''dbo'' ' +
   CASE WHEN @populatedTableExists = 1 
		THEN 'AND [name] COLLATE Latin1_General_BIN IN (SELECT TableName COLLATE Latin1_General_BIN FROM [dbo].[Populated] WHERE [ColumnName] IS NULL AND [Populated] = 1) '
		ELSE ''
...
BEGIN
	...
	'SELECT c.[name], p.[name], ' + CAST(@iter AS NVARCHAR(10)) +
	 ...
	 'WHERE t.[name] = ''' + @tableName +
	   ...
	   'AND p.[name] != ''sysname'' ' +
	CASE WHEN @populatedTableExists = 1 
	 	 THEN ' AND c.[name] COLLATE Latin1_General_BIN IN (SELECT [ColumnName] COLLATE Latin1_General_BIN FROM [dbo].[Populated] WHERE [TableName] = ''' + @tableName + ''' AND [ColumnName] IS NOT NULL AND [Populated] = 1)'
		 ELSE ' '
	END +	   
