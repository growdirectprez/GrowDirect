...
BEGIN
	...
	BEGIN
		...
		BEGIN
			...
			SELECT @IncludeInHadouken = 1, @isDefaultDisplayColumn = 0, @isHidden = 0, @commonfield = 0
			...
			IF @columnName IN ('TransactionID', 'StoreNo', 'POSNo', 'TicketNo', 'EndTransDateTime', 'CashierNo', 'TradingDay')
			SELECT @commonField = 1

			INSERT INTO @columnData 
			SELECT @tableID, @tableName, @tableAlias, @columnName, @columnDataType, @maxVal, @minVal, @columnName, @IncludeInHadouken, @isDefaultDisplayColumn, @isHidden, @commonField
		END
	...
	BEGIN
		INSERT INTO @columnData
		SELECT @tableID, @tableName, @tableAlias, 'TransactionTime', 'time', 0, 0, 'TransactionTime', 1, 0, 0, 0
	END
	...
	BEGIN
		...
		BEGIN
			INSERT INTO @columnData 
			SELECT @tableID, @tableName, @tableAlias, @columnName, @columnDataType, @maxVal, @minVal, @columnName, 1, 0, 0, 0
		END
...
SELECT A.TableID AS "@TableId", A.TableName AS "@TableName", A.TableAlias AS "@TableAlias",
	(SELECT ColumnName AS "@ColumnName", DataType AS "@DataType", Alias AS "@Alias", MaxVal AS "@MaxVal", MinVal AS "@MinVal", IncludeInHadouken AS "@IncludeInHadouken", IsDefaultDisplayColumn AS "@IsDefaultDisplayColumn", IsHidden AS "@IsHidden", IsCommon AS "@IsCommon"
