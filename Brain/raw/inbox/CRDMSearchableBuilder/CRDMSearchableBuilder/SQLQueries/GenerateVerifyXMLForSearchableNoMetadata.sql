...
'SELECT [name] ' +
  'FROM sys.tables ' +
   'WHERE [name] IN (''return_history'',''return_item'',''return_item_type'',''return_type'',''override_transaction'',''return_type_txt'') ' +
   'AND SCHEMA_NAME(schema_id) = ''dbo'' ' +
 'ORDER BY [name]'
...
BEGIN
	...
	BEGIN
		...
		BEGIN
			...
			BEGIN 
				IF @columnName IN ('transaction_id', 'consumer_id', 'operator_id', 'returndatetime', 'return_amount')
				BEGIN
			...
			SELECT @tableID, @tableName, @tableName, @columnName, @columnDataType, @maxVal, @minVal, @columnName, 0, @isDefaultDisplayColumn
		END
...
SELECT A.TableID AS "@TableId", A.TableName AS "@TableName", A.TableAlias AS "@TableAlias",
 ...
 ORDER BY A.TableID
   FOR XML PATH('Table'), ROOT('Appriss.Verify'), TYPE
