...
- *Legal / compliance hold* — legal or compliance team places a hold pending regulatory review, labeling dispute, or safety evaluation that does not rise to recall level; chain-wide until released
...
**RaaS tier.** Item eligibility and listing-based authorization is available at all subscription tiers. Regulatory zone overlay (blue laws, age restrictions, hours restrictions) is Operations Agent — Standard tier. Operational block management with expiry protocols and escalation routing is Operations Agent — Standard tier. Real-time recall propagation as contract events (no POS parameter update step) requires the Verified Vendor tier plus Operations Agent — Standard. Cross-site authorization health monitoring and stale-configuration alerting is Operations Agent — Standard tier. AI-assisted regulatory zone maintenance (detecting regulatory changes relevant to a site's jurisdiction and surfacing configuration update candidates) is Operations Agent — Premium tier.
