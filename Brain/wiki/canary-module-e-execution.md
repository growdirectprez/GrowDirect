...
type: wiki
status: active
tags: [canary, retail-spine, module-e, execution, exception-detection, case-management, cross-domain, v3]
sources:
...
# Canary Module — E (Execution)

## Summary

E (Execution) is the cross-domain exception detection and case management surface. **v3 design — implementation deferred.** This wiki article is the Canary-specific crosswalk for the v3 E module. The canonical, vendor-neutral module spec lives at `Canary-Retail-Brain/modules/E-execution.md`.

E is the capstone of the Canary Retail Spine. It generalizes Q's (Loss Prevention) Chirp+Fox pattern across all domains: inventory, commercial, pricing, labor, and space. E detects exceptions everywhere and correlates them. When a customer appears in both a fraud alert (Q) and a return-abuse pattern (P), E connects them in one case, not two. E is the retail operating system's immune system.
...
E would write to the `app` schema (configuration and master data). Anticipated tables:

| Table | Owner | Purpose |
|---|---|---|
| `exceptions` | E | All detected exceptions: id, merchant_id, source_module, rule_id, severity (info/warning/critical), signal_data (JSON), auto_escalate_flag, case_id_if_escalated |
| `exception_rules` | E | Rule catalog per domain: id, domain (Q/S/P/L/D/J/F), rule_id, rule_name, description, severity_threshold, auto_escalate_flag |
| `cases` | E | Case aggregation: id, merchant_id, case_type (multi_domain_exception), status, investigator_id, created_at (inherited Fox model) |
| `case_exceptions` | E | Junction: case_id, exception_id (one case can span multiple exceptions) |
| `case_subjects` | E | Subjects of interest per case: case_id, subject_type (customer/employee/vendor), subject_id, role_in_case |
| `case_timeline` | E | Append-only audit log: id, case_id, action_type, actor_id, timestamp, details (inherited Fox model) |
| `case_evidence` | E | INSERT-only evidence: id, case_id, evidence_type (file/screenshot/log), source_module, file_hash, chain_hash (inherited Fox model) |
| `remediation_requests` | E | Requests to modules for remediation: id, case_id, target_module, remediation_type (refund/restock/adjustment/payroll_correction), status |

E reads from (no write):

| Table | Owner | Why |
|---|---|---|
| All exception streams | Q, S, P, L, D, J, F | E subscribes to all domains' exception publishers |
| Subject identities | R, L | E looks up customer/employee records for subject context |

## SDD crosswalk

No v3 SDDs exist yet for E. Canary's current SDDs only cover v1 modules (T, Q, architecture, data-model) and projected v2 modules (C, D, F, J).

Projected SDD structure (future):
- `Canary/docs/sdds/v3/execution.md` — cross-domain exception detection, rule evaluation, auto-escalation, case aggregation, evidence inheritance from Fox
...
| Axis | Cell | Notes |
|---|---|---|
| [[../projects/RetailSpine\|Retail Spine]] | v3 Capstone | E is the final ring of the spine; it reads all other modules |
| [[../projects/RetailSpine\|Retail Spine]] Ledger roles | Reconciler + Cross-domain Analyzer | E reads all spine outputs; publishes aggregated exception cases |
| Reference implementation | Q (Loss Prevention) | E generalizes Q's Chirp+Fox pattern across all domains |
| Upstream feeder | E reads from Q, S, P, L, D, J, F (all spine modules) | |
...
3. **Remediation automation.** When E detects a pricing error, should it automatically route to P for correction, or create a case for manual approval? Current assumption: manual approval (case) at v3; automated routing deferred to v3.1.
4. **Subject correlation timing.** When correlating exceptions by subject (customer/employee), what time window should E use? One hour? One day? Configurable? Current assumption: configurable; default 24 hours.
5. **Evidence inheritance from Fox.** Should E reuse Q's Fox evidence schema (hash-chained, INSERT-only, trigger-enforced), or design a new E-specific evidence model? Current assumption: inherit Fox model at v3.
