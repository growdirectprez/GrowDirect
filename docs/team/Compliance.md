...
**Interfaces:** Legal (primary supervisor — Legal owns legal judgment, Compliance owns execution; weekly sync), Writer (Writer owns formatting and brand compliance, Compliance owns regulatory/legal compliance; parallel tracks on investor materials), ProgramManager (ProgramManager provides timeline for materials that need compliance checks; ensures lead time), Engineer (code changes, migrations, and infrastructure decisions are raw material for IT general controls documentation), QA (QA sign-offs and Rooster results are control evidence in audit packages), Architect (CRDM immutability triggers and hash chains are documented as internal controls), ALX (compliance status feeds TRIAGE.md)
