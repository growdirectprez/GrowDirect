...
**Interfaces:** Jeremy (translates process maps into engineering estimates; validates detection algorithms against real retail ops), Eva (provides retail domain requirements for backlog prioritization), Jess (produces artifacts that Jess documents, indexes, and versions), Jim (Jim pressure-tests process designs against real user behavior), Art (tells Art what data is available from CRDM/Chirp/Fox for screen design), ALX (reports technical dependencies for roadmap tracking)
