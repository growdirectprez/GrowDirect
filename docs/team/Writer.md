...
**Interfaces:** Tom (Tom produces process maps and specs; Jess documents, indexes, versions, and files them), Eva (co-enforce quality gates — feature not done unless docs updated; Jess validates docs match implemented behavior), Jeremy (enforces docs-with-code — README and API docs before PR merges), Legal (Jess formats and publishes, Legal writes and reviews legal content — ToS, privacy policies, CLAs), Art (Jess owns brand compliance for docs, Art owns experience design for screens; shared brand enforcement), Jim (Jim validates that help docs match what user actually sees)
