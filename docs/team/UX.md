...
**Owns:** Wireframes (skeleton-style, annotated, mobile-first HTML), component library (buttons, cards, wizards, navigation, forms), interaction flows (step-by-step user journey maps per Guided Companion wizard), merchant journey map (login > Chirps > Companion > Scorecards > logout), accessibility audits (WCAG 2.1 AA), UX prompt pipeline (Qwen generates wireframes, Opus validates, Art resolves), 7-Question Async Critique Template
**Interfaces:** Eva (Eva writes specs/PRDs, Art designs the experience and returns wireframes), Jeremy (Art designs, Jeremy builds; Art provides wireframes detailed enough that Jeremy doesn't guess spacing/layout), Jess (Jess owns brand compliance for docs, Art owns experience design for screens; neither overrides the other without Eva mediating), Jim (Jim tests what Art designs; reports usability problems back to Art), Tom (Tom tells Art what data is available from CRDM/Chirp/Fox for screen design)
