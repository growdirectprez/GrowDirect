...
Canary connects to a merchant's Square account via OAuth 2.0, ingests transaction data in real time via signed webhooks (HMAC-verified), and runs 37 detection rules across 10 categories: payment anomalies, cash drawer variances, refund patterns, void abuse, timecard fraud, gift card manipulation, discount abuse, loyalty fraud, inventory shrinkage, and employee behavioral patterns.
...
Thirty-seven rules across ten categories. Each rule is a question the data answers: Is this refund pattern normal? Is this drawer variance a trend? Is this employee's void rate an outlier? The AI collapses the analyst interpretation layer that enterprise systems require. The product is the analyst.
...
## V.3 — The Credit Model

No tiers. No monthly subscription. One wallet, one balance, operations cost what they cost.

Every merchant gets a sat-denominated credit wallet at onboarding — a custodial account where GrowDirect holds the keys. This is Wallet 1: promotional credits for platform operations, not merchant commerce funds. The wallet denominates in satoshis because the entire protocol stack denominates in satoshis. The unit of account matches the unit of inscription.

**Treasury-funded onboarding.** When a merchant completes Square OAuth and their account is provisioned, GrowDirect's treasury deposits 100,000 sats into their wallet. No credit card required. No trial period. No "freemium" gate. The merchant connects their Square account and starts using the platform immediately. GrowDirect is the first payer — the treasury absorbs the onboarding cost because proving value before asking for money is how you earn trust with a small business owner who has been burned by every SaaS vendor that promised and underdelivered.

**Tunable gas schedule.** Every metered operation on the platform has a sat cost defined in a `gas_schedule` table — a database-driven fee schedule that can be adjusted without code changes, without deployments, without downtime. Three categories of metered operations:

- *Transaction processing* — each Square event ingested, normalized, and stored in the CRDM. The base unit of platform activity.
- *Detection events* — each Chirp rule evaluation that produces an alert. The merchant pays for signal, not noise.
- *Compute operations* — Fox case management actions, Owl AI analysis passes, health check reports. Premium intelligence that costs real compute to produce.

**Gold list alerts are always free.** Three detection rules surface at zero cost, no matter the wallet balance: Post-Void (C-502), Off-Clock Transaction (C-301), and Untendered Order (C-204). These are the highest-signal alerts in the system — the ones that catch the clearest theft patterns. They are the hook. A merchant who connects their Square account and sees a Post-Void alert on day one understands immediately what Canary does. The credit system funds everything else. The gold list funds trust.

**Soft degradation on credit exhaustion.** When a merchant's wallet runs low, the platform does not shut off. The dashboard stays live. Gold list alerts keep firing. What gates progressively are the premium layers: Fox case management, Owl AI analysis, scheduled health checks. The merchant sees exactly what they are missing and exactly what it costs to restore. The HTTP response is 402 Payment Required — with a Lightning invoice QR code to refill. No sales call. No upgrade page. Scan, pay, resume.

**Self-service Lightning refills.** Credit purchases flow through the Strike API. The merchant scans a QR code with Cash App, Strike, Phoenix, or any Lightning-compatible wallet. The payment settles in seconds. Credits appear in their wallet immediately. The refill flow is the same whether the merchant is topping up $5 or $500 — scan, confirm, done. No minimum purchase. No billing cycle. No accounts receivable.

**L402 macaroon authentication.** At onboarding, the platform mints an L402 macaroon scoped to the merchant's UUID with a 30-day expiry. Every API request presents the macaroon. The server verifies the macaroon's validity; the wallet table authorizes the spend. The macaroon is the credential. The wallet is the authorization. The two are decoupled so that authentication (who are you?) and authorization (can you afford this?) are independent concerns — the same architecture that Lightning Service Authentication Tokens established, applied to a retail data platform.

**Break-even economics.** The model is usage-proportional, not tier-dependent. At N merchants each processing approximately 3,000 transactions per month, gas revenue — 1 sat per transaction processed plus detection event fees plus compute operation fees — covers infrastructure. The economics scale linearly with merchant activity. A merchant who processes 500 transactions pays less than one who processes 10,000. Neither is subsidizing the other. The gas schedule is the price list. The wallet is the meter. The Lightning Network is the settlement rail.
...
# APPENDIX C — What Is Live vs. Building Now vs. Designed vs. Future Architecture

*For every claim in this document, the reader should know: is this running code, near-term deliverable, designed roadmap, or future architecture?*

| Status | Description | Examples |
|---|---|---|
| **LIVE TODAY** | Running code. Real data. Proven. | Square OAuth 2.0 self-authorization, webhook HMAC-SHA256 signature verification, TSP pipeline (4 stream consumers, 37 detection rules across 10 categories), evidence seal layer (Sub 1) with INSERT-only enforcement and hash chains, Chirp detection engine with configurable thresholds per merchant, Fox case management with evidence locker, Owl AI analysis with Ollama integration, dashboard + alert pipeline + risk scoring, 60+ database models across app/sales/metrics schemas |
| **BUILDING NOW (Sprint 7)** | Active development. Current sprint. | Goose credit system (merchant wallets, gas schedule, Strike API integration), L402 macaroon authentication and authorization, treasury funding pipeline, credit-based feature gating with soft degradation |
| **DESIGNED (ROADMAP)** | Architecture specified. Not yet built. | OrdinalsBot inscription bridge (Bitcoin anchor for evidence), L402 validation gate (public API — pay sats to verify events), Genesis Pool minting |
| **FUTURE ARCHITECTURE** | Roadmap. Design principles established. | DAO governance, Kubernetes auto-purchase, cross-vertical expansion (healthcare, supply chain), heartbeat network |

> **Fig. 14 — Appendix C Status Map**
>
> Current state of the elJeffe protocol mapped across four deployment horizons. LIVE TODAY: Square OAuth, webhook verification, TSP with 37 rules across 10 categories, evidence seal layer, Chirp engine, Fox case management, Owl AI, dashboard and alert pipeline, 60+ models. BUILDING NOW: Goose credit system, L402 macaroons, treasury funding, credit-based gating. DESIGNED: OrdinalsBot inscription bridge, L402 validation gate, Genesis Pool. FUTURE: DAO governance, Kubernetes auto-purchase, cross-vertical expansion.

```mermaid
flowchart TD
    subgraph LIVE["LIVE TODAY ✅"]
        direction TB
        L1["Square OAuth 2.0<br/>self-authorization"]
        L2["Webhook HMAC-SHA256<br/>signature verification"]
        L3["TSP pipeline: 4 consumers<br/>37 rules · 10 categories"]
        L4["Evidence seal layer (Sub 1)<br/>INSERT-only · hash chains"]
        L5["Chirp detection engine<br/>configurable thresholds"]
        L6["Fox case management<br/>+ evidence locker"]
        L7["Owl AI analysis<br/>Ollama integration"]
        L8["Dashboard · alerts<br/>risk scoring"]
        L9["60+ database models<br/>app · sales · metrics"]
    end
    subgraph S7["BUILDING NOW (Sprint 7) 🔧"]
        direction TB
        S7A["Goose credit system<br/>merchant wallets + gas schedule"]
        S7B["L402 macaroon<br/>authentication"]
        S7C["Treasury funding<br/>pipeline"]
        S7D["Credit-based feature gating<br/>soft degradation"]
        S7E["Strike API<br/>Lightning refills"]
    end
    subgraph DES["DESIGNED (ROADMAP) 🏗️"]
        direction TB
        DESA["OrdinalsBot inscription<br/>bridge"]
        DESB["L402 validation gate<br/>public API"]
        DESC["Genesis Pool<br/>minting"]
    end
    subgraph FUT["FUTURE ARCHITECTURE 🗺️"]
        direction TB
        FUT1["DAO governance"]
        FUT2["Kubernetes<br/>auto-purchase"]
        FUT3["Cross-vertical expansion<br/>healthcare · supply chain"]
        FUT4["Heartbeat network<br/>always-on monitoring"]
    end
    LIVE -->|"Sprint 7<br/>delivers"| S7
    S7 -->|"Spec<br/>complete"| DES
    DES -->|"Scale<br/>demands"| FUT
    style LIVE fill:#1e8449,stroke:#fff,color:#fff
    style S7 fill:#2980b9,stroke:#fff,color:#fff
    style DES fill:#7d3c98,stroke:#fff,color:#fff
