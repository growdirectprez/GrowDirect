...
-- i.inventory_documents — Header for receipts/transfers/counts/RTVs (single table, type-discriminated)
...
-- i.inventory_lots — Lot/serial/expiry tracking (optional; only used by lot-tracked merchants)
...
-- i.inventory_document_lines — Per-line detail with generated variance column
...
-- i.inventory_positions — Current SOH per item × location (with reserved/on-order/in-transit)
...
CREATE INDEX idx_positions_unsynced ON i.inventory_positions(last_movement_at) WHERE last_count_at IS NULL;

-- i.inventory_movements — Append-only event log per atomic SOH change (all movement types unified)
